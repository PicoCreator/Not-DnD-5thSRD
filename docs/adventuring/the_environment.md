# The Environment 
Exploring the unknown can be a dangerous endeavor, and the rules in this section cover some of the most important ways in which adventurers interact with the environment.

## Falling 
When a creature falls from a great height, it takes 1d6 bludgeoning damage for every 10 feet it fell, up to a maximum of 20d6. The creature lands prone, unless it manages to avoid taking damage from the fall. 
...
A character's interaction with objects in an environment is often simple to resolve in the game. The player tells the GM what his or her character is doing, such as moving a lever, and the GM describes what, if anything, happens.

For example, a character might decide to pull a lever, which might, in turn, raise a portcullis, cause a room to flood with water, or open a secret door in a nearby wall. If the lever is stuck in position, though, a character might need to force it. In such a situation, the GM might call for a Strength check to see whether the character can wrench the lever into place. The GM sets the DC for any such check based on the difficulty of the task.
